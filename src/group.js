...
async function loadFoodItems() {
  ...
  try {
    ...
    groupFoodDiv.innerHTML =
      '<p class="my-2 error rounded-md py-1 px-2">Error loading food items.</p>';
  }
}

function createFoodElement(foodId, foodData) {
  const expDate = foodData.expDate
    ? new Date(foodData.expDate).toLocaleDateString()
    : "-";

  const foodDiv = document.createElement("div");
  foodDiv.classList =
    "flex flex-row justify-between items-center py-2 px-2 border-b border-[--secondary-border-color]";
